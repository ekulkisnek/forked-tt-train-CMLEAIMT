...
#include "models.hpp" // Assuming this file will contain the NanoGPT class definition
...
int main(int argc, char **argv) {
    ...
    //Adapt to new model architecture
    GPTConfig gpt_config; // Populate gpt_config appropriately
    auto model = std::make_shared<NanoGPT>(gpt_config); //Using the new NanoGPT model
    ...
    if (!model_path.empty() && std::filesystem::exists(model_path)) {
        ...
        load_model_and_optimizer(model_path, model, optimizer, "transformer", "adamw"); //Adapt if necessary
    ...
    for (uint32_t epoch = 0; epoch < num_epochs; ++epoch) {
        for (auto [features, target, masks, positions] : train_dataloader) {
            auto start_timer = std::chrono::high_resolution_clock::now();
            optimizer.zero_grad();
            auto output = (*model)(features, positions, masks); //Adapt if necessary
            ...
            if (!model_path.empty() && global_step % model_save_interval == 0) {
                save_model_and_optimizer(model_path, model, optimizer, "transformer", "adamw"); //Adapt if necessary
            }
